use crate::{
    ...
    world::neighborhood::GenericNeighborhood,
};
...
impl StructureType {
    pub fn get_structures(
        &self,
        blocks: &GenericNeighborhood<Blocks>,
        ...
        match self {
            StructureType::Tree => {
                ...
                VolumetricRange::new(min..max, min..max - 1, min..max)
                    ...
                    .filter(|(x, y, z)| noise_3d.at(*x, *y, *z).unwrap() <= &TREE_PROBABILITY)
                    .map(|(x, y, z)| {
                        let local_noise = noise_3d.at(x, y, z).unwrap();
